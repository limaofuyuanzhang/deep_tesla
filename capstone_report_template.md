...
　　无人驾驶近年来发展非常迅速，主要依靠车内的智能驾驶系统配合车外的众多传感器来实现无人驾驶的目标。而前置摄像头则是众多传感器之一。
　　该项目基于"MIT 6.S094: Deep Learning for Self-Driving Cars"。在该项目中，我们利用MIT 6.S094 这门公开课中的Tesla行驶数据集中前置相机所拍摄的路况图像，训练深度学习模型，利用该模型对车辆转向角度的预测。
...
该项目是一个监督回归问题。训练数据集中对应每一帧图像都给定了相应的方向盘转向角度。模型的目标是学习不同驾驶路况下方向盘的角度，以达到可以正确地应对一个未遇过的驾驶路况。
...
$MSE=\dfrac {1} {N}\sum _{i=1}^{N}\sqrt {y_{p}-y}$
